...
// TOOL 1: Add Task
// ==========================================
server.tool("add_task", "Add a new task to the accountability tracker.", {
...
// TOOL 2: Get Tasks
// ==========================================
server.tool("get_tasks", "Fetch tasks from the database.", {
...
// ==========================================
server.tool("complete_task", "Mark a task as complete. Calculates the penalty if late.", {
    ...
    if (task.status !== "pending")
        return { content: [{ type: "text", text: `Task is already ${task.status}.` }], isError: true };
...
// TOOL 4: Delete Task
// ==========================================
server.tool("delete_task", "Permanently delete a task.", {
    task_id: z.string().describe("UUID of the task"),
}, async ({ task_id }) => {
...
// ==========================================
server.tool("get_current_time", "Get the exact current date and time in IST (Indian Standard Time). Call this before adding tasks if the user uses relative time (like 'tomorrow' or 'in 2 hours') to ensure accuracy.", async () => {
    const now = new Date();
